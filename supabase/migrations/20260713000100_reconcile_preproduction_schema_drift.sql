-- Reconcile known drift in the authoritative pre-production project.
--
-- This migration is intentionally safe for both a fresh baseline replay and
-- the linked pre-production schema. It is not applied remotely by this task.
...
ALTER FUNCTION public.restore_locked_jobs_for_premium_user(uuid, text, integer, text[])
  OWNER TO postgres;
