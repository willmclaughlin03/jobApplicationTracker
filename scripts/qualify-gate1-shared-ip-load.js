...
function preparation(profile, env) {
  return {
    ...
    provider: { existingPreproductionProjectOnly: true, newDependencies: false,
      ...
      reconciliationBudget: 'uses unused direct requests while reserving one deletion per attempted create',
      cancelledCreateSettlementMaxMs: profile.timeoutMs,
      cleanupWorstCaseMs: (profile.sessions + Math.ceil(profile.sessions / 5)) * profile.timeoutMs },
    ...
    cleanup: 'reconciles uncertain creates using bounded provider pages and both ownership tags before deleting confirmed owned IDs; graceful interruption keeps cleanup active',
    runMarker: 'credential-free ownership tags persisted before provisioning in .tmp/gate1-runs/<run UUID>.json',
    interruptedCleanup: 'incomplete reconciliation or hard termination can leave accounts; use the persisted gate1_run plus gate1_qualification metadata for separately approved exact-account recovery',
