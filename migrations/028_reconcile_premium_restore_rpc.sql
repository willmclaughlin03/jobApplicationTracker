-- Reconcile the Premium restore RPC on databases that already applied the
-- original three-argument migration 019 definition.
--
-- Purpose:
--   - install the hardened four-argument function without relying on an
--     already-applied migration file being replayed
--   - remove the stale overload that does not receive the Premium price
--     allowlist
--   - reassert the service-role-only execution boundary and refresh PostgREST

DO $$
BEGIN
  IF pg_catalog.to_regclass('public.jobs') IS NULL THEN
    RAISE EXCEPTION 'public.jobs must exist before reconciling premium restore RPC'
      USING ERRCODE = '42P01';
  END IF;

  IF pg_catalog.to_regclass('public.billing_subscriptions') IS NULL THEN
    RAISE EXCEPTION 'public.billing_subscriptions must exist before reconciling premium restore RPC'
      USING ERRCODE = '42P01';
  END IF;

  IF pg_catalog.to_regprocedure('public.resolve_canonical_storage_status_for_user(uuid)') IS NULL THEN
    RAISE EXCEPTION 'resolve_canonical_storage_status_for_user(uuid) must exist before reconciling premium restore RPC'
      USING ERRCODE = '42883';
  END IF;
END;
$$;
...
BEGIN
  IF pg_catalog.to_regprocedure(
    ...
    RAISE EXCEPTION 'required four-argument premium restore RPC is missing after reconciliation'
      USING ERRCODE = '42883';
  END IF;
  ...
  IF pg_catalog.to_regprocedure(
    ...
    RAISE EXCEPTION 'stale three-argument premium restore RPC remains after reconciliation'
      USING ERRCODE = '42710';
  END IF;
  ...
  IF pg_catalog.has_function_privilege(
    ...
    RAISE EXCEPTION 'premium restore RPC execution privileges are not service-role only'
      USING ERRCODE = '42501';
  END IF;
END;
$$;
