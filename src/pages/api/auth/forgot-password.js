  ...
  try {
    ...
    if (redirectTo) {
      ...
      try {
        ...
        req.log.warn('Rejected malformed redirect URL in forgot-password', { redirectTo });
        return sendError(res, 400, 'VALIDATION_ERROR', ERROR_MESSAGES.VALIDATION_ERROR);
      }

      const isAllowed = allowedOrigins.some(origin => parsedOrigin === origin);
      if (!isAllowed) {
        req.log.warn('Rejected redirect URL in forgot-password', { redirectTo });
        return sendError(res, 400, 'VALIDATION_ERROR', ERROR_MESSAGES.VALIDATION_ERROR);
      }
    }
    ...
    req.log.error({ err: error }, 'Forgot-password service error');
  }
